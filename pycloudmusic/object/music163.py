...
class _Music(DataObject, Music163Comment):
    ...
    async def play(
        ...
        return data["data"][0]["url"] if type == 'url' else await _download(data["data"][0]["url"], f"{self.id}.mp3",
                                                                            download_path)
...
class My(User):
    ...
    async def recommend_songs(self) -> Generator[Music, None, None]:
        """获取日推"""
        return (Music(music_data) for music_data in
                (await _post("/api/v3/discovery/recommend/songs"))["data"]["dailySongs"])

    async def recommend_resource(self) -> Generator[ShortPlayList, None, None]:
        """获取每日推荐歌单"""
        return (ShortPlayList(playlist_data) for playlist_data in
                (await _post("/api/v1/discovery/recommend/resource"))["recommend"])
