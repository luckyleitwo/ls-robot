...
from cloudmusic import Music163Api
import asyncio
...
async def main():
    ...
    # CustomMusicS = CustomMusic(Music)
    print(await Music.music(ids=65528))
    my = await Music.my()
    user = await Music.user(my.id)
    likeList = await user.like_music()

    # playlist = await Music.playlist(likeList)
    # 迭代 PlayList 打印歌单曲目
    for music in likeList:
        print(music)
        print(music.name, music.artist_str, music.id)
    music = await Music.music(ids=65528)
    play = await music.play(download_path='../download')
    print(play)
    # player.play(
    #     '/Users/shenglei/ls/ls-robot/download/65528.mp3',
    #     not True,
    # )
